...
import { agencyApi, CRMOverviewResponse } from '@/lib/api/agency.api'
...
export default function CRMOverviewPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const overview = await agencyApi.getCRMOverview();
        setData(overview);
      } catch (error) {
    ...
    <div className="flex flex-col gap-8 p-4 sm:p-8 bg-white min-h-screen">
      ...
      <div className="flex flex-col gap-1">
        <p className="text-[#64748B] text-sm sm:text-base">
          Organization-wide client relationship management insights
        </p>
      </div>
      ...
      <div className="space-y-6">
        <h2 className="text-[18px] font-bold text-[#0F172A]">Recent CRM Activity</h2>
        ...
        <div className="space-y-3">
          {data.recent_activities.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-[#64748B] bg-[#F8F9FA] rounded-[16px] border border-[#E2E8F0]">
